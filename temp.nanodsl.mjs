...
geometry {
  ...
  item =
    | geometry -- geometry
    | any      -- default
  geometry = str<"geometry"> ws ":" ws "{" ws x y width height as? rectangle? "}" ws "," ws
...
geometry : function (_geo,ws1,_colon,_ws5,lb,ws2,x,y,w,h,as,rect,rb,ws3,_comma,ws4,) {
enter_rule ("geometry");
    set_return (`\n${_geo.rwr ()}${ws1.rwr ()}${_colon.rwr ()}${_ws5.rwr ()}${lb.rwr ()}${ws2.rwr ()}${x.rwr ()}${y.rwr ()}${w.rwr ()}${h.rwr ()}${as.rwr ().join ('')}${rect.rwr ().join ('')}${rb.rwr ()}${ws3.rwr ()}${_comma.rwr ()}${ws4.rwr ()}`);
